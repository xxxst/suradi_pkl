<section class="content">
  <div class="col-md-12">
    ...
    <div class="card card-dark">
      ...
      <form action="<?=site_url('suratkeluar/proses')?>" method="POST" enctype="multipart/form-data">
        <div class="card-body">
            ...
            <div class="form-group <?=form_error('penandatanganan')?'has-error': null?>">
                ...
                <input type="text" name="penandatanganan" class="form-control" id="nomorhp" placeholder="Penandatanganan" required value="<?= $row->penandatanganan ?>">
                <?= form_error('penandatanganan','<div class="text-small text-danger">','</div>') ?>
            </div>
            <div class="form-group <?=form_error('jenissurat')?'has-error': null?>">
                <label for="jenissurat">Jenis Surat</label>
                <select name="jenissurat" id="jenissurat" class="form-control" required>
                    <option value="">- Pilih -</option>
                    <option value="001" <?= $row->jenissurat=='001'?'selected': ''?>>001 - Surat Undangan</option>
                    <option value="470" <?= $row->jenissurat=='470'?'selected': ''?>>470 - Surat Kuasa</option>
                </select>
                <?= form_error('jenissurat','<div class="text-small text-danger">','</div>') ?>
            </div>
            ...
            <div class="form-group <?=form_error('nama_file')?'has-error': null?>">
                ...
                <input type="file" name="nama_file" class="form-control" id="nama_file" required value="<?=base_url('uploads/surat_keluar/'.$row->nama_file)?>" placeholder="nama_file">
