<section class="content">
  <div class="col-md-12">
    ...
    <div class="card card-dark">
      ...
      <form action="<?=site_url('permintaankabid/proses')?>" method="POST" enctype="multipart/form-data">
        <div class="card-body">
            ...
            <div class="form-group <?=form_error('tujuan')?'has-error': null?>">
              ...
              <div class="form-group <?=form_error('nama_file')?'has-error': null?>">
                ...
                <input type="file" name="nama_file" class="form-control" id="nama_file" placeholder="Nama File" value="<?= $row->nama_file ?>">
                <small>(Silahkan <?=$page=='edit'? 'Mengganti Jika Ada Revisi Di ' : 'Upload' ?> File Surat)</small>
                <?= form_error('keterangan','<div class="text-small text-danger">','</div>') ?>
              </div>
              <div class="form-group <?=form_error('nama_file')?'has-error': null?>">
                <label for="ket_verifikator2">Keterangan Revisi</label>
                <textarea type="text" name="ket_verifikator2" class="form-control" id="ket_verifikator3" required placeholder="Keterangan Revisi"><?= $row->ket_verifikator2 ?></textarea>
                <?= form_error('keterangan','<div class="text-small text-danger">','</div>') ?>
              </div>
              <div class="form-group <?=form_error('verifikator2')?'has-error': null?>">
                <label for="verifikator2">verifikasi</label>
                <select name="verifikator2" id="verifikator2" class="form-control" required>
                    <option value="">- Pilih -</option>
                    <option value="0" <?= $row->verifikator2=='0'?'selected': ''?>>Belum Terverifikasi</option>
                    <option value="1" <?= $row->verifikator2=='1'?'selected': ''?>>Terverifikasi</option>
                </select>
                <?= form_error('keterangan','<div class="text-small text-danger">','</div>') ?>
            </div>
        </div>
