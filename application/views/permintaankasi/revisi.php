<section class="content">
  <div class="col-md-12">
    ...
    <div class="card card-dark">
      ...
      <form action="<?=site_url('permintaankasi/proses')?>" method="POST" enctype="multipart/form-data">
        <div class="card-body">
            ...
            <div class="form-group <?=form_error('tujuan')?'has-error': null?>">
              ...
              <div class="form-group <?=form_error('nama_file')?'has-error': null?>">
                <label for="ket_verifikator1">Keterangan Revisi</label>
                <textarea type="text" name="ket_verifikator1" class="form-control" id="ket_verifikator1" required placeholder="Keterangan Revisi"><?= $row->ket_verifikator1 ?></textarea>
                <?= form_error('keterangan','<div class="text-small text-danger">','</div>') ?>
              </div>
              <div class="form-group <?=form_error('verifikator1')?'has-error': null?>">
                <label for="verifikator1">Verifikasi</label>
                <select name="verifikator1" id="verifikator1" class="form-control" required>
                    <option value="">- Pilih -</option>
                    <option value="0" <?= $row->verifikator1=='0'?'selected': ''?>>Belum Terverifikasi</option>
                    <option value="1" <?= $row->verifikator1=='1'?'selected': ''?>>Terverifikasi</option>
                </select>
                <?= form_error('keterangan','<div class="text-small text-danger">','</div>') ?>
            </div>
        </div>
